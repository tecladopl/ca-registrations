    ...
    <x-slot name="header">
        <div class="flex h-16 justify-between">
            <div
                ...
                <span class="inline-flex items-center">{{ __('Answers') }}</span>
            </div>
        ...
        <div class="mx-auto">
            <div class="bg-white overflow-hidden shadow-xl">
                <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
                    ...
                    <hr>
                    <div>
                        @livewire('answer.index', ['questionnaire' => $questionnaire], key(time().$questionnaire->id))
                        <hr>
                        @livewire('answer.add', key(time().$questionnaire->id))
                        <hr>
                        @livewire('answer.add-predefined', key(time().$questionnaire->id))
                    </div>
                </div>
            </div>
        </div>
