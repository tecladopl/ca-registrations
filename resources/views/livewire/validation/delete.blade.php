<div>
    ...
    <x-jet-dialog-modal wire:model="modalDelete">
        <form wire:submit.prevent="save">
            <x-slot name="title">
                {{ __('Deleting Validation') }}
            </x-slot>
            <x-slot name="content">
                <p>{{ __('Validation') }}: {{$validation->rule_name}}</p>
                <p>{{__('Are you sure?')}}</p>
            </x-slot>
            <x-slot name="footer">
                ...
                <x-jet-danger-button  type="submit" class="ml-2" wire:click="deleteValidation" wire:loading.attr="disabled">
                    {{ __('Delete') }}
                </x-jet-danger-button>
            </x-slot>
        </form>
